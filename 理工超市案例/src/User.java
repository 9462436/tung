...
public class User {
    ...
    public void login() {
        ...
        if (supermarket.userManager.loginValidate(username,passward));   //this.name.equals(username) && this.passward.equals(passward)
    ...
    public int register() {
        ...
        if (number != random)
            ...
            boolean res= supermarket.userManager.addUser(cus);
            if(res)
                System.out.println(this.name+"注册成功！");
            else
                System.out.println(this.name+"注册失败！");
            m++;
        }
        return m;
    }
